...
import {
  ...
  resetToDefaults,
  recalculate,
} from "../../stores/redux/calculatorSlice";
...
function SavingsAndDepositCalculatorWithReduxInner(): React.JSX.Element {
  const dispatch = useAppDispatch();
  const { principal, annualRate, months, frequency, schedule, error } = useAppSelector(selectCalculator);
  const [liveMessage, setLiveMessage] = useState("");

  // Recalculate whenever inputs change
  useEffect(() => {
    dispatch(recalculate());
  }, [dispatch, principal, annualRate, months, frequency]);
      ...
      <main id="main-content" className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 mb-8 transition-all duration-200 hover:shadow-md">
          ...
          <CalculatorForm
            ...
            frequency={frequency}
            error={error}
            onPrincipalChange={(value) => dispatch(setPrincipal(value))}
            onAnnualRateChange={(value) => dispatch(setAnnualRate(value))}
            onMonthsChange={(value) => dispatch(setMonths(value))}
            onFrequencyChange={(value) => dispatch(setFrequency(value))}
          />
        </div>
  ...
  return (
    <CalculatorErrorBoundary>
      <Provider store={store}>
        <SavingsAndDepositCalculatorWithReduxInner />
      </Provider>
    </CalculatorErrorBoundary>
  );
}