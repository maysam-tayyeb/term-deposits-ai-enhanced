...
import { calculatorStore, calculatorActions } from "../../stores/valtio/calculatorStore";
...
function SavingsAndDepositCalculatorWithValtioInner(): React.JSX.Element {
  const snapshot = useSnapshot(calculatorStore);
  const [liveMessage, setLiveMessage] = useState("");

  // Announce calculation results
  useEffect(() => {
    if (!snapshot.error && snapshot.schedule.length > 0) {
      const finalBalance = snapshot.schedule[snapshot.schedule.length - 1].balance;
      const totalInterest = snapshot.schedule[snapshot.schedule.length - 1].interest;
      setLiveMessage(
  ...
  }, [snapshot.schedule, snapshot.error]);
      ...
      <main id="main-content" className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 mb-8 transition-all duration-200 hover:shadow-md">
          <div className="flex justify-between items-center mb-8">
            ...
            <button
              type="button"
              onClick={() => calculatorActions.resetToDefaults()}
          ...
          <CalculatorForm
            ...
            onPrincipalChange={(value) => calculatorActions.setPrincipal(value)}
            onAnnualRateChange={(value) => calculatorActions.setAnnualRate(value)}
            onMonthsChange={(value) => calculatorActions.setMonths(value)}
            onFrequencyChange={(value) => calculatorActions.setFrequency(value)}
          />
        </div>
        ...
        {snapshot.error ? (
          ...
          snapshot.schedule.length > 0 && <ResultsDisplay schedule={[...snapshot.schedule]} />
        )}
      </main>
    </div>
  );
...
export function SavingsAndDepositCalculatorWithValtio(): React.JSX.Element {
  return (
    <CalculatorErrorBoundary>
      <SavingsAndDepositCalculatorWithValtioInner />
    </CalculatorErrorBoundary>
  );
}