...
test.describe('Accessibility Tests', () => {
  ...
  test('should maintain focus visibility when navigating with keyboard', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="interest-rate-input"]')).toBeFocused();
    
    // Tab to investment term input
    await page.keyboard.press('Tab');
    await expect(page.locator('[data-testid="investment-term-input"]')).toBeFocused();
  ...
  test('should support keyboard navigation in button group', async ({ page }) => {
    await page.goto('http://localhost:5173');
    ...
    const monthlyButton = page.locator('[data-testid="radio-re-invest-monthly"]');
    await monthlyButton.focus();
    await expect(monthlyButton).toBeFocused();
    
    // Test arrow key navigation
    await page.keyboard.press('ArrowRight');
    const quarterlyButton = page.locator('[data-testid="radio-re-invest-quarterly"]');
    await expect(quarterlyButton).toBeFocused();
    
    await page.keyboard.press('ArrowRight');
    const annuallyButton = page.locator('[data-testid="radio-re-invest-annually"]');
